...
> Stop switching between Obsidian notes. One page. Everything you need. Memo your thoughts, crush your todos, track your projects — and make it look incredible doing it. [【中文版】](README.md)
...
- **Media size memory** — Images/videos sections remember their thumbnail-size choice across reloads and restarts
- **Pinned card properties** — Pick exactly which frontmatter fields show on cards (common keys pinned first, vault-scanned keys, plus custom additions), complementing the max-count fallback
- **Folder config: tag search** — A search box in the tag filter section for quick location when the vault has many tags
- **Fixes** — Duplicate close button on the note popover under Obsidian 1.13+; calendar task insert position, day-number rendering, path picker, and weather symbol mapping hardening

### 2.1.2
- **Island becomes the default theme** — New installs start in the Animal Crossing pastel palette; two new default banner images
- **Pomodoro heatmap moved up** — In the stats modal the heatmap now sits above the today timeline
- **Scroll preserved on edit** — In-place refreshes after card edits no longer jump back to the top

### 2.1.1
- **Habit backfill** — A clock icon on the habits widget opens a backfill dialog for yesterday's check-ins (tick individually or select all missed)
- **New todos on top** — Newly added todo items land at the top of the list
- **Banner spacing** — Tighter gap between the banner and content

### 2.1.0
- **Multi-workspace** — Several dashboards in one vault: each workspace has its own dashboard file and config; a numeric switcher at the banner's top-left jumps between them; the settings page offers drag-reorder, renaming, and path redirection, on mobile too

### 2.0.0
- **Tighter grouped views** — Denser gutters in grouped library/folder views fit more cards per screen
- **Toolbar right cluster** — Count and pagination controls align to the right edge uniformly
- **New community group QR**
