...
- **Memo card drag lag and afterimage** — Dragging a memo card to a new spot felt sluggish and left a brief visual ghost. The move triggered a full-board re-render (every section, every card, with each memo line re-parsing links), which is what the user waited on, and it landed during the dragged card's `transform` settle transition — tearing the half-rotated card into a double image. Card moves are now optimistic: only the affected section(s) are rewritten in place while the data persists in the background, so the drop is instant and no full re-render interrupts the transition.
