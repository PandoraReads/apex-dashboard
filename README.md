...
### 📅 日历（Calendar）
原生月历格子，展示全仓库所有带日期的任务（无需 dataview 或外部插件）。每格列出当天任务，点击格子查看当日日程；可打开全屏大月历，支持月份切换与就地勾选。多日行程（`[start::]`/`[end::]`）跨天显示。侧栏日历小组件的全屏放大视图也可以作为**日历分区**直接嵌入工作台：完整月视图（多日横条）与周时间轴网格、任务筛选、今日跳转一应俱全，点击日期数字即可为该日添加待办（自动写入日记或工作台待办清单）。
...
### 🌐 网页（Web）
在分区内嵌入任意网页：填入网址即可像内嵌浏览器面板一样浏览。嵌入引擎全自动——预检站点的嵌入策略，允许 iframe 的站点直接嵌入；拒绝的站点在桌面端自动改用独立网页视图（webview，自带持久登录态，Google Keep、滴答清单网页版等登录型应用可用），手机端则显示兜底卡片并提供"在浏览器中打开"。显示比例提供 50%–125% 预设按钮（密集 Web 应用缩小显示）。移动端建议嵌入 https 站点；嵌入以浏览为主，弹窗与下载请在系统浏览器中进行。

### ⚡ 快捷操作
将常用快捷方式固定到侧边栏，支持两种操作类型：**文件**链接可打开任意文档，**命令**快捷方式可触发任意 Obsidian 命令。内置新建日记和新建笔记预设。

### 🌤️ 侧栏小组件
左侧栏提供 11 种可独立开关、可拖拽排序的小组件：

- **天气** — 实时天气（温度、体感、湿度、风速）+ 5 日预报（每日图标与最高/最低温）。基于 Open-Meteo，无需 API key；城市搜索自动补全精确定位
- **农历黄历** — 农历日期、传统节日与宜忌速览
- **番茄钟** — 专注计时器：活动标签、专注/休息循环、今日活动分布甜甜圈图；统计页提供目标进度、效率分、趋势图、12 周热力图与今日时间线
- **阅读追踪** — 豆瓣搜书或手动添加书目，计时阅读并按页码记录进度；书卡显示封面、作者与进度条
- **倒计时** — 自定义多个目标日期，按天/小时显示剩余
- **年度进度** — 今年已流逝百分比一览
- **日历** — 月/周双视图日历网格，汇总全库带日期任务；点日期看日程、可直接添加任务（自动写入当日日记），多日事件横跨显示
- **习惯打卡** — 每日勾选打卡，支持昨日补卡、连续天数与热力图统计
- **记账** — 快捷记录收支（备注、补录、自定义类目），今日小计与周/月/年/全部统计，明细账本支持筛选、编辑与 CSV 导入导出
- **相册** — 像电子相框一样自动轮播指定文件夹的图片：悬停暂停、左右手动切换与序号角标，1:1 / 3:4 面板比例，渐隐、左推、右推、放大淡入四种切换动画，可选是否递归子文件夹；轮播间隔 3–60 秒可调，文件夹内图片增删改名后原地刷新、不打断当前播放位置
- **音乐** — 侧栏搜索并播放网易云音乐免费歌曲（可在设置中登录网易云账号，按账号权限播放会员歌曲），支持粘贴歌单链接一键导入、音量与循环模式、播放列表持久化（封面懒加载补全），播放时可选显示全局浮动迷你播放条（关闭仪表盘也能控制；仅桌面端）

侧栏还常驻 **周历条**（7 天紧凑条、高亮今天）、**最近文档** 与 **滚动回顶** 按钮；手机端有同款小组件条，小组件数据跨设备同步合并。
...
### 🧩 自定义分区
共 **14 种分区类型**自由组合 — **待办**、**备忘**、**便利贴**、**笔记（项目卡）**、**无封面笔记**、**数据视图**、**数据库**、**文件夹**、**图片**、**视频**、**日历**、**微信读书**、**滴答清单**、**网页** — 每种类型都有独立的布局与交互，打造专属工作流。
...
| 主题 | 风格 |
|------|------|
| **大地** | 托斯卡纳暖阳：32 秒慢速暖光漂移的羊皮纸底，陶土强调色配橄榄点缀 |
| **北欧** | 极夜峡湾：霜青冰蓝的极光缓漂冷白底，冰川青强调色 |
| **宝莉** | 小马宝莉风高饱和多彩：夜空底六色彩虹极光渐变，每个分区一匹小马的专属色 |
| **花漾** | 玫瑰色柔光，透明分区无边框 |
| **丁香** | 印象派庭院：28 秒慢速漂移的花境水彩底，半透明象牙卡片，藤紫强调色 |
| **岛屿**（默认） | 动物森友会风格柔和色调，森林绿配海洋蓝 |
| **苔原** | 冷灰底 + 牛油果绿极光，鼠尾草绿玻璃卡片 |
| **抹茶** | 茶碗美学：26 秒缓漂的茶庭洗地，米色陶瓷卡片配深抹茶强调色与金茶点缀 |
| **朱砂** | 墨韵极简：宣纸底与松烟墨的黑白搭配，40 秒墨晕缓漂，朱砂印章点睛 |
| **霓虹** | 近黑底 + 霓虹品红高亮，亮暗模式一致 |
| **伏特** | 近黑底 + 电光青高亮，亮暗模式一致 |
| **岩浆** | 暖近黑底 + 岩浆橙高亮，亮暗模式一致 |
| **曜黑** | 纯黑底配柠檬高亮，亮暗模式一致 |
...
### 3.6.3
- **数据库/文件夹分区四视图分组** — 网格、画廊、列表、表格视图的工具栏新增「分组」按钮：可选择不分组、按文件夹（扫描目录下的一级文件夹）或按任意 frontmatter 属性分组；分组维度保存在看板文件中，各分区独立记忆；看板视图沿用配置弹窗里原有的分组设置
- **分组一键展开/折叠** — 开启分组后，数据库/文件夹分区工具栏新增展开/折叠按钮：一键收起或展开全部分组，按钮图标与提示随当前状态切换
- **小组件区域宽高可拖拽** — 左右布局下侧栏右缘新增拖拽条，可在 180–420px 间调整宽度并自动记忆；上下布局下小组件条支持拖拽调整高度（240–560px）；习惯与阅读小组件高度档位（全高 / 三分之二 / 二分之一 / 三分之一）可在设置中自选
- **音乐小组件播放修复** — 网易云偶发的 -110 版权校验闪断改为间隔重试后再判定，会员曲目不再被误跳；确认无法播放时弹出提示点名歌曲与原因（会员、购曲或地区版权受限）后跳下一首，不再静默连跳
- **待办一键归档可选当天日记** — 设置新增「归档位置」：除写入指定文件外，可选择写入当天日记（当天无日记时自动按核心「日记」插件的模板创建）；无法定位日记时中止并提示，不会误归档
- **备忘卡保存支持自定义模板** — 备忘/便利贴卡片的「保存为笔记」支持自定义模板
- **刷新性能修复** — 仓库文件变动现在只重建扫描范围覆盖该路径的分区：在别处编辑笔记、RSS 投递落盘、图片变动不再连带闪刷全部数据库/文件夹分区；分区查询结果未变化时直接跳过重建。实测范围外编辑与图片噪声的无效重建从每轮 3 个分区降为 0，RSS 投递只刷新对应的资讯分区
- **滚动位置全局保持** — 完整重渲染前快照整个页面的滚动状态（上下布局小组件卡座的横向滚动、竖向滚动区、各分区卡列、任务列表、小组件内部列表），渲染后精确回放；分区或小组件重排后仍按锚点恢复原位——在哪里操作，界面就停在哪里，不再跳回开头

### 3.6.2
- **主题工作室配色方案与颜色预设** — 每个颜色区域新增配色方案下拉（跟随主题 / 浅色 / 深色 / 自定义，与小组件背景的前景配色同款配方），支持逐字段预设快速取色
- **笔记分区新笔记** — 封面/无封面笔记分区的卡片操作行新增「新笔记」按钮；分区设置新增新笔记模板与保存文件夹（留空为仓库根目录）
- **备忘卡 Markdown 渲染** — 备忘与便利贴分区的卡片正文改用 Obsidian 渲染器渲染 Markdown（逐行保守适配，既有备忘外观不变）
- **修复** — 文件夹分区设置保存时「新笔记模板」被静默丢弃；模板 frontmatter 现合并在筛选属性之下，原始行保留

### 3.6.1
- **分区工具栏改为下拉单按钮** — 数据库/文件夹/图片/视频分区的视图切换与卡片大小（S/M/L）不再整排铺开：各显示一个当前项按钮，点击弹原生菜单选择，选中后按钮即切换；工具栏安静了一大截
- **图片/视频分区显示范围** — 设置弹窗新增「显示范围」：多选仓库文件夹（含选择器），选定后仅显示范围内文件（排除文件夹仍在范围内生效），留空为全仓库
- **分区新笔记模板** — 文件夹/数据库分区配置弹窗新增「新笔记模板」：选定模板笔记后，工具栏「+」创建的新笔记以模板正文起始（自动替换 {{title}} 与 {{date:…}}），分区筛选属性照常预填；模板失效自动降级为空白笔记
- **日历待办落点指定** — 日历设置新增「待办落点」：默认维持原链（当天日记 → 工作台待办），也可指定固定文件（按开头/结尾插入）或文件夹（每天一篇 YYYY-MM-DD 自动创建追加）
- **快捷按钮全主题玻璃化** — 大地/北欧/岛屿/苔原/抹茶/丁香/朱砂/宝莉的按钮底色与边框统一为主题强调色染色的透明玻璃（对齐花漾/霓虹/电光/熔岩/曜黑），十二主题质感统一
- **主题工作室颜色透明度 + 页面底色修复** — 每个颜色项旁新增透明度滑杆（0–100%）；自定义页面底色现在真正生效（水彩主题的渐变背景也会被替换），透明度 0 可直接透出 Obsidian 系统背景
- **小组件面板毛玻璃内框** — 设置背景图后，卡片内自动浮起一块独立磨砂玻璃（真模糊 + 主题卡片色填充 + 顶缘高光），文字与图分层清晰；番茄钟小组件加入背景图支持（齿轮在卡片右上按钮簇）
- **多项体验修正** — 折叠书签伸缩隐藏（悬停垂下）；快捷按钮调色板按钮移除（背景图+前景配色取代）；习惯打卡右上按钮常显；自定义外观颜色支持文字图标前景配色（浅色/深色/取色器）修复快捷按钮文字不变色；修复农历小组件无法拖拽排序（异步刷新后拖拽失联，改为事件委托）；修复 Windows 双排分区拖拽宽度失效（配对伙伴脏值换行 + 亚像素换行 + webview 吞事件三重根因）

### 3.6.0
- **全新上下布局** — 设置 → 常规 → 布局新增图示化选择：左右分栏（原布局）或上下堆叠——小组件按高度比例（全高 / 三分之二 / 二分之一 / 三分之一）拼成卡片墙置于横幅下方，闪念工作条置顶固定，小组件与分区间页滚动，图钉折叠照旧；手机端不受影响
- **小组件背景图与前景配色** — 纪念日、倒计时、年度倒计、音乐、习惯、快捷按钮支持自定义背景（图片选择器只列图片，支持网络图片链接），透明度/黑色蒙版/模糊度可调，浅色/深色/取色器前景配色；卡片右上角齿轮不离开工作台即可换图
- **新增纪念日小组件** — 历史日期计时（年月日 / 总天数 / 天+小时三档精度），每年同日提醒，原生日期选择器
- **相册小组件多实例** — 可添加任意数量，每个独立选择文件夹与卡片尺寸
- **修复：Windows 双排分区拖拽宽度失效** — 配对行宽度脏值换行、亚像素精度、嵌入页吞鼠标事件三重根因全部修复（指针捕获拖拽守卫）

### 2.5.4
- **分区内新建笔记** — 文件夹/数据库分区工具栏新增「新建笔记」按钮：文件夹分区直接在分区配置的文件夹内创建（配置多个文件夹时在点击处弹菜单选择）；数据库分区保存到「设置 → 常规 → 数据库分区新建笔记保存路径」（可浏览选文件夹，留空为仓库根目录），并自动把分区筛选条件写入 frontmatter（equals/contains 取首值、tags 汇成列表），新笔记创建即命中筛选、自动出现在分区中；无法预填的条件（notEquals、路径/日期、同属性冲突）会弹提示列出属性名。标题弹窗可取消，重名自动加 `-2` 后缀，文件夹不存在自动创建；手机/平板/桌面通用
- **双排分区自定义宽度** — 并排成对的两个分区之间新增拖拽分隔条，左右拖动即可调整各自占比（20–80%，松手即保存到看板文件）；拖动期间嵌入网页降载防误触。桌面端功能
- **记账一二级分类与历史统计** — 「管理类目」中可将类目归入一级分类，账目明细表出现「一级」列与筛选，统计弹窗的分类占比环形图与排行榜可切换一级/二级聚合；「历史」档支持任意历史周/月/年翻页查看（历史周/月按日、历史年按月出趋势图），KPI 环比自动对齐上一同周期

### 2.5.3
- **类目排序与一级分类** — 「管理类目」弹窗支持拖拽排序（内置类目也可调，录入下拉与明细筛选顺序同步；附上移/下移按钮，手机端同样可用）；新增一级分类层：在管理弹窗新建一级并给类目归组后，账目明细表出现「一级」列与一级筛选（含未分组），统计弹窗的分类占比环形图与排行榜可一键切换「一级/二级」聚合；归组关系独立保存，调整归组即全部历史自动重新分桶，记录数据零改动
- **统计新增历史周/历史月** — 统计弹窗「历史」档内置 周/月/年 子切换与 ‹› 翻页：任意历史周（标签显示起止日期）、历史月、历史年均可查看；KPI 环比自动对齐上一同周期，历史周/月按日、历史年按月出趋势图；翻页至当前周期与最早有数据的年份自动止步
- **修复：统计卡片百分比挤压数字** — 记账与番茄钟统计的 KPI 卡片改为数值独占一行居中、环比百分比换行居中显示在下一行，不再互相挤压或超出卡片边框

### 2.5.2
- **音乐播放断流自愈** — 偶发播放失败不再弹「播放出错，跳到下一首」并永久跳歌：改为静默换新链接原曲重试一次（播放中途断流按原进度续播；暂停太久链接过期也自动换新再续），重试仍失败才安静跳到下一首。只保留三种可操作提醒：手动点 VIP 歌、登录失效、连续多首失败熔断停播
- **Banner 名言字体改为下拉选择** — 不用手输字体名了：下拉直接挑——中文字体 8 款（楷体、宋体、仿宋、隶书、行楷、圆体、黑体、霞鹜文楷），英文字体 8 款（Georgia、Times、Palatino、Baskerville、Garamond、Didot、Helvetica、Courier）；每项内置跨平台字体栈，macOS / Windows / iPad 同一设置直接生效；此前手输过的字体名会保留在下拉里，不会丢

### 2.5.1
- **网易云账号登录** — 桌面版在「设置 → 小组件 → 音乐」打开网易云官方页面登录（手机号或扫码），登录后按账号权限播放：会员、购曲曲目完整播放。凭据直接提交网易云，会话仅保留在本机 Electron 独立分区，不写入插件配置、不随仓库同步；退出登录即清除。匿名使用一切如旧
- **跨分区移动卡片** — 拖动卡片标题栏，即可在待办清单、备忘录、便利贴之间移动卡片；备忘/待办互转时保留任务与勾选状态，笔记链接随行；有/无封面笔记卡片也可移入这三个分区
- **Banner 名言字体** — 名言颜色旁新增自定义字体：填入本机字体名（如 楷体、Georgia）即刻生效，名言与作者行一同应用；重置即回到主题默认
- **手机端数据库/文件夹分区加高** — 分区行高 50vh→60vh、工具栏瘦身（计数/每页条数收起），网格视图在主流手机可显示 3-4 张卡片；平板与桌面不受影响
- **修复：拖拽书籍偶发消失** — 拖动落点判定与写入序列加固，跨分区拖拽后刷新/重载不再丢失卡片

### 2.5.0
- **相册小组件** — 指定一个仓库文件夹即可像电子相框一样自动轮播其中图片：悬停暂停、左右手动切换（手动后重新计时，不会连着跳两张）、右下角序号角标；面板比例 1:1 或 3:4 竖版，切换动画四选一（渐隐 / 左推 / 右推 / 放大淡入，双层交替实现），轮播间隔 3–60 秒可调，默认递归子文件夹；起始张按时间播种（不会每次都停在第一张），下一张自动预载；文件夹内图片增删改名后原地刷新，当前照片幸存则不打断播放位置
- **音乐小组件** — 侧栏搜索并播放网易云音乐免费歌曲：可在设置中登录网易云账号，按账号权限播放会员歌曲，粘贴歌单链接一键导入，音量与循环模式可调，播放列表持久化（封面懒加载补全），播放时可选显示全局浮动迷你播放条（关掉仪表盘也能控制；仅桌面端）
- **修复：小组件内控件拖动误触发排序** — 侧栏小组件此前整体设为可拖拽，从音量滑块、输入框、下拉或按钮上按下拖动会被识别成"拖动排序"（音乐面板调音量时最明显）；现在改为按下时按手势判定——落在表单控件与按钮上就不武装拖拽，落在面板空白处才可拖动排序，各小组件一致生效
- **公告弹窗** — 标题改为「新增 2 个分区+2 个小组件」，内容补上相册与音乐两个小组件的介绍

### 2.4.1
- **朱砂主题（原「墨白」改版并更名）** — 根治纯黑白搭配的丧礼感：宣纸暖白底与松烟墨蓝黑替代纯中性灰阶，40 秒超慢墨晕缓漂（全主题最慢），朱砂印章红作为唯一强调色（链接、复选框、焦点、进度条），引用竖条保持墨色、卡片添一缕纸感投影——黑白搭配原样保留，气质从灵堂变成书房
- **主题更名** — 「墨白 / Mono」正式更名「朱砂 / Vermilion」，存量安装的选择无缝保留
- **主题顺序调整** — 花漾、丁香移至宝莉之后，朱砂移至抹茶之后；设置下拉、循环切换命令与主题表三处同步

### 2.4.0
- **五款主题大改版** — 宝莉、丁香、大地、北欧、抹茶全部获得慢速漂移的洗地背景动画（26–32 秒，呼吸感而非炫技；移动端与减少动态偏好下自动停漂），并补上不透明弹窗底色保险（杜绝亮色 Obsidian 下弹窗白底白字）
- **宝莉（原「极光」）** — 小马宝莉风高饱和多彩：深夜空底上双轮六色彩虹极光，每个分区一匹小马的专属标题色（闪念=萍琪派热粉、待办=苹果杰克琥珀、笔记=小蝶奶油黄、项目=云宝天蓝、书库=暮光紫……），卡片悬停泛同色光晕，进度条为糖果双色渐变
- **丁香** — 印象派庭院：藤紫/玫瑰/象牙水彩在 28 秒里缓缓流动，半透明象牙卡片浮于花境之上，藤紫强调色取代与底色难分的灰紫
- **大地** — 托斯卡纳暖阳：蜜金/陶土/橄榄暖光以 32 秒爬过羊皮纸底，分区改为暖玻璃、卡片保持奶油实色，强调色升级为陶土红
- **北欧** — 极夜峡湾：霜蓝/冰青/靛蓝/薄荷的极光缓漂冷白底，强调色换为冰川青，告别 slate 死灰实心板
- **抹茶** — 茶碗美学：茶庭洗地 + 半透明米色陶瓷卡片，深抹茶强调色与底色明确分离（原来两者几乎同色），金茶作点缀
- **全局修复** — 添加分区/微信读书等 4 处硬编码紫色选中态改为按各主题强调色自动派生，全部主题受益

### 2.3.0
- **日历分区** — 侧栏日历的放大视图直接嵌入工作台：完整月视图（多日事件横跨横条）与周时间轴网格、月/周切换、任务筛选与今日跳转；悬停日期即可速览当天全部待办，点击日期数字直接添加任务（自动写入日记或工作台待办清单）；待办自动汇总全库，笔记变更原地刷新、不打断当前浏览
- **网页嵌入分区** — 把常用网页钉进工作台：填入网址即可嵌入，引擎全自动——预检站点嵌入策略，允许 iframe 的站点直接嵌入，拒绝的站点在桌面端自动改用网页视图（自带持久登录态，Google Keep、滴答清单网页版适用），手机端显示兜底卡片并可一键在浏览器打开；显示比例 50%–125% 预设按钮一键选择
- **修复：删除确认弹窗按 Enter 屏幕逐层变黑** — 弹窗此前不接管键盘与焦点，回车会反复触发底下仍聚焦的删除按钮、叠出多层半黑遮罩；现在弹窗打开即聚焦安全默认按钮，Enter/Escape 行为明确，不再叠层

### 2.2.1
- **快捷按钮小组件** — 快捷操作更名为「快捷按钮」，正式加入侧栏小组件体系：可在设置中开关、与其他小组件一样拖动排序；侧栏固定图钉移至 Banner 左下角（桌面端，手机端布局不变）
- **快捷按钮配色** — 新增调色板按钮，可分别自定义组件背景色与按钮颜色（暖色预设 + 自定义取色）；按钮改为两列等宽网格 + 小圆角，面板高度 180px、隐藏滚动条
- **设置页三标签** — 设置重构为「常规 / 小组件 / Buy Me a Coffee」横向标签页；微信读书与滴答清单留在常规页；新增 Buy Me a Coffee 打赏页
- **快捷筛选改版** — 弹窗更名「快捷筛选」，移除文件夹筛选，单个日期按钮 + 日历面板直接框选起止区间（不透明底色），筛选生效时仅按钮高亮、工具栏不再显示筛选内容
- **修复** — 待办卡片内向下拖拽排序多跳一位的问题；设置页搜索框宽度/间距、数字输入框高度、日历添加行输入等高、微信读书与番茄钟设置卡等宽等多项样式修复
- **公告弹窗改为内容驱动** — 版本升级不再触发公告弹窗，只有公告内容真正更新时才会再次显示
...
- **主题精简 14 → 13** — 移除薄雾、翡翠、日食；存量安装自动迁移到最接近的主题（薄雾→电光、翡翠→抹茶、日食→朱砂），不会回退默认
...
- **新增两款主题：墨白与曜黑** — 新增 Mono（墨白，纯黑白极简；现更名为「朱砂」，墨韵极简：宣纸松烟墨配朱砂印章点睛）和 Onyx（曜黑，纯黑配柠檬黄强调色，明暗模式完全一致）。同时移除了春日（Prism）主题
...
在桌面版的「设置 → Apex Dashboard → 小组件 → 音乐」选择「手机号 / 网易云登录」，在官方页面选择手机号登录并完成验证；如网易云限制手机号入口，请按官方页面使用扫码登录。此入口不提供独立短信发送接口。登录成功后返回音乐小组件播放。会员有效期、单独购曲及地区版权均由网易云校验，试听片段不会作为完整歌曲播放。

手机号、密码或验证码直接提交给网易云官方页面。插件仅使用网易云登录会话请求账号和音乐地址；凭据保存在本机 Obsidian 的独立 Electron 会话中，不写入 data.json，也不随仓库同步。退出登录会停止播放并清除该本机会话。登录失效时请重新登录。无需第三方代理服务。

### 跨分区移动卡片

拖动卡片标题栏，可在待办清单、备忘录和便利贴之间移动卡片。有封面及无封面的笔记卡片也可移入这三个分区。

- 移入备忘录：变为备忘卡片，显示原文字、任务、勾选状态及笔记链接。
- 移入待办清单：变为待办卡片，原有任务保留；其他文字按非空行转成未完成条目，笔记链接也作为条目保留。
- 移入便利贴：保留原形态；笔记卡片保留有封面／无封面的区别。

跨区后会重新绘制卡片并更新操作入口，保存、重新打开仍保持转换结果。移动的是工作台卡片，不移动或删除其链接的笔记文件。
