...
- **音乐** — 侧栏搜索并播放网易云音乐免费歌曲（无需登录，VIP 曲目自动跳过并提示），支持粘贴歌单链接一键导入、音量与循环模式、播放列表持久化（封面懒加载补全），播放时可选显示全局浮动迷你播放条（关闭仪表盘也能控制；仅桌面端）
...
- **音乐小组件** — 侧栏搜索并播放网易云音乐免费歌曲：无需登录、VIP 曲目自动跳过并提示，粘贴歌单链接一键导入，音量与循环模式可调，播放列表持久化（封面懒加载补全），播放时可选显示全局浮动迷你播放条（关掉仪表盘也能控制；仅桌面端）
