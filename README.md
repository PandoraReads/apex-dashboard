...
### 🌐 网页（Web）
在分区内嵌入任意网页：填入网址即可像内嵌浏览器面板一样浏览。**自动模式**会预检站点的嵌入策略——允许 iframe 的站点直接嵌入；拒绝的站点在桌面端自动改用独立网页视图（webview，自带持久登录态，Google Keep、滴答清单网页版等登录型应用推荐此模式），手机端则显示兜底卡片并提供"在浏览器中打开"。也可手动锁定 iframe / 网页视图模式，并支持 0.5–2 倍缩放（密集 Web 应用缩小显示）。移动端建议嵌入 https 站点；嵌入以浏览为主，弹窗与下载请在系统浏览器中进行。
...
- **网页嵌入分区** — 把常用网页钉进工作台：填入网址即可嵌入。自动模式预检站点嵌入策略——允许 iframe 的站点直接嵌入，拒绝的站点在桌面端自动改用网页视图（自带持久登录态，Google Keep、滴答清单网页版适用），手机端显示兜底卡片并可一键在浏览器打开；支持手动锁定 iframe/网页视图模式与 0.5–2 倍缩放
